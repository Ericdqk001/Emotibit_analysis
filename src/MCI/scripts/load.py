"""Load parsed EmotiBit sensor data for MCI.
...
def discover_recordings(base_path: Path) -> list[RecordingInfo]:
    ...
    Structure: base_path/day{1,2}/INTACT-MCI-XX/
    """
    recordings = []

    for day_dir in sorted(base_path.glob("day*")):
        for participant_dir in sorted(day_dir.glob("INTACT-MCI-*")):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python load.py <emotibit_base_path>")
        print("\nExample:")
        print("  python load.py /Volumes/INT-ACT/INTACT-MCI_restructured/emotibit")
